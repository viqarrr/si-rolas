...
interface SchoolContactsCreateProps {
    existingContacts: SchoolContact[];
}
...
const SchoolContactsCreate: React.FC<SchoolContactsCreateProps> = ({ existingContacts = existingContact }) => {
    ...
    const existingTypes = existingContacts.map((contact) => contact.type);

    const contactTypeOptions = [
        { value: ContactType.ADDRESS, label: 'Address' },
        ...
        { value: ContactType.YOUTUBE, label: 'YouTube' },
    ].filter((option) => !existingTypes.includes(option.value as any));

    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        post(route('school-contacts.store'));
    };
                        ...
                        <Card>
                            ...
                            <CardContent className="space-y-4">
                                <div className="space-y-2">
                                    ...
                                    <Select value={data.type} onValueChange={(value: SchoolContact['type']) => setData('type', value)}>
                                        <SelectTrigger>
                                            <SelectValue placeholder="Select contact type" />
                                        </SelectTrigger>
                                        <SelectContent>
                                            {contactTypeOptions.length === 0 ? (
                                                <div className="px-2 py-1 text-sm text-muted-foreground">All contact types are already in use</div>
                                            ) : (
                            ...
                            <CardFooter>
                                <div className="flex w-full justify-end gap-2 pt-6">
                                    ...
                                    <Button type="submit" disabled={processing || contactTypeOptions.length === 0}>
                                        {processing ? 'Creating...' : 'Save Contact'}
                                    </Button>
                                </div>
                            </CardFooter>
                        </Card>
